...
build-capi-windows: capi-setup
	TARGET_DIR=target/x86_64-pc-windows-gnu/release
	RUSTFLAGS="${RUSTFLAGS} -Ctarget-feature=+crt-static" $(CARGO_BINARY) build --target x86_64-pc-windows-gnu --manifest-path lib/c-api/Cargo.toml --release \
		--no-default-features --features wat,universal,dylib,staticlib,wasi,middlewares $(capi_compiler_features)
...
package-capi:
	...
	if [ -f target/x86_64-pc-windows-gnu/release/wasmer.dll ]; then \
		...
		cp target/x86_64-pc-windows-gnu/release/wasmer.dll.a package/lib/wasmer.dll.a ;\
	fi
	...
	if [ -f $(TARGET_DIR)/libwasmer.dylib ]; then \
		cp target/x86_64-pc-windows-gnu/release/libwasmer.dylib package/lib/libwasmer.dylib ;\
	fi
